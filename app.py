...
def upload_video():
    ...
    if request.method == 'POST':
        ...
        # temp_path = basepath + "/temp_folder"
        ...
        # subprocess.call(['ffmpeg', '-i', file_path, '-vf', 'fps=30', 'temp_folder/output%06d.jpg'])

        # # crop images in temp_folder
        # cropper = Cropper(width=256, height=256)
        # for filename in os.listdir("temp_folder"):
        #     full_path = os.path.join(temp_path, filename)
        #     cropped_array = cropper.crop(full_path)
        #     if cropped_array is not None:
        #         # save cropped image
        #         cropped_image = Image.fromarray(cropped_array)
        #         cropped_image.save(full_path)
        #     else:
        #         # if frame does not have a face detected, delete the frame
        #         os.remove(full_path)

        # # convert cropped images to video
        # os.remove(file_path)
        # subprocess.call(['ffmpeg', '-framerate', '30', '-pattern_type', 'glob', '-i', 'temp_folder/*.jpg', '-vcodec', 'libx264', file_path])
