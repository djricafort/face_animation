...
class Akira():
    ...
    def generate_video(driving_video, source_image):
        config = "config/vox-256.yaml"
        checkpoint = "checkpoint/vox-cpk.pth.tar"
        find_best_frame = False
        best_frame = None
        filename = "static/output/fake_{:%Y%m%dT%H%M%S}.mp4".format(datetime.datetime.now())
        result_video = filename
        ...
        if find_best_frame or best_frame is not None:
            ...
            predictions_forward = make_animation(source_image, driving_forward, generator, kp_detector, relative=True, adapt_movement_scale=True, cpu=False)
            predictions_backward = make_animation(source_image, driving_backward, generator, kp_detector, relative=True, adapt_movement_scale=True, cpu=False)
